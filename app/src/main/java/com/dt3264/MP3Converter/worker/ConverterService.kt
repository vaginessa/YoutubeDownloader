...
import com.dt3264.MP3Converter.*
...
import com.dt3264.MP3Converter.util.NotificationHelper
import com.dt3264.MP3Converter.util.*
...
class ConverterService : Service() {
    companion object {
        ...
        fun newJob(
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                context.startForegroundService(newJobIntent)
            } else {
    ...
    override fun onCreate() {
        ...
        notificationUpdateDisposable = jobManager
                ...
                .subscribe(this::updateNotificationIfNeeded)
    ...
    private fun onJobStatusChanged(job: Job) {
        if (job.status == JobStatus.COMPLETED) {
            ...
            val uri = URI(job.command.inputs[0])
            val file = File(uri)
            ...
            val args: Array<String> = arrayOf(file.absolutePath)
            val contRes: ContentResolver = baseContext.contentResolver
            //then we remove the downloaded file from the mediaStore
            contRes.delete(filesUri, where, args)
        }
    ...
    private fun updateNotificationIfNeeded(outputSize: String) {
        if (inForeground) {
            ...
            if (outputSize.isNotBlank()) {
                val duration = outputSize.split(" ")[1].replace("%","").toInt()
                notificationBuilder.setContentText(getString(R.string.converter_service_running_with_speed, outputSize))
                        .setProgress(100, duration, false)
            } else {
